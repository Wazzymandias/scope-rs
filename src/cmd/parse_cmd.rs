...
use crate::farcaster::time::{FARCASTER_EPOCH, farcaster_to_unix};
...
fn timestamp_from_sync_id(input: &Vec<u8>) -> eyre::Result<DateTime<Utc>>{
    ...
    match c {
        ...
        _ => {
            Err(eyre!("failed to parse timestamp"))
        }
    }
}
...
impl SyncIdCommand {
    pub fn execute(&self) -> eyre::Result<()> {
        ...
        let bytes: Vec<u8> = parts.iter().map(|part| {
        ...
        let source_sync_ids: Result<SyncIds, serde_json::Error> = serde_json::from_reader(source_reader);
        source_sync_ids.and_then(|sync_ids| {
